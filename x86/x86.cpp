...
int main(){
  ...
  string tmpindex ; // 把table.table的數字讀掉 
    ...
    switch( i ) {
      case 0: // 讀table1
        for ( int j = 1 ; !fin.eof() ; j++ )
          fin >> tmpindex >> t1[j] ;
        for ( int k = 1 ; k < 150 ; k++ ) { // 將此table內的資料轉為大寫
  ...
  // strcpy(inputname, "x86_input.txt") ; // 測試用
...
int isWhat(string token) {
  ...
  bool isDigit = false ;
  for ( int i = 0 ; i < len ; i++) {
    if ( !isdigit(ary[i]) && i != len-1 ) {
      if ( ary[i] != 'A' && ary[i] != 'B' && ary[i] != 'C' &&
           ary[i] != 'D' && ary[i] != 'E' && ary[i] != 'F' ) // 為了16進位做處理 
        isDigit = false ;
      else
        isDigit = true ;
    }
      
    if ( i == len-1 && ary[i] == 'H') // 為了16進位做處理  
      isDigit = true ;
  }
